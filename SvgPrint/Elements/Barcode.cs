// part of the source code:  Michael Nattfalk
// Original source:
// https://github.com/nattfalk/BarcodeGenerator/blob/master/BarcodeGenerator/Code128GS1/BarcodeImage.cs


using System;
using System.Xml.Serialization;
using SvgPrint.Barcode;
using SvgPrint.Model;

namespace SvgPrint.Elements
{
    public class Barcode : IPageDataContent
    {
        ...
        {
            var value = data.GetType().GetProperty(DataField).GetValue(data) as string;

            var encoder = new Encoder();
            var barcodeData = encoder.Encode(value);

            var height = 35;
            var scale = .46;
            var fontSize = 4m;
            var content = this.CreateBarcode(barcodeData, 1, height * 1.95m);


            content = $@"<g transform='translate({pageInfo.CurrentX} {pageInfo.CurrentY}) scale({scale} {scale})'>
                {content}
                </g>";

            pageInfo.CurrentY += height;
            content += $"<text x='{pageInfo.CurrentX}' y='{pageInfo.CurrentY}' font-size='{fontSize}'>{value}</text>";
        ...
        private int[] BarcodeStringToGs1Array(string barcode)
        {
            return null;
        }
        ...
        private string CreateBarcode(int[] codes, int BarWeight, decimal height)
        {
            int width;
            ...
            for (int codeidx = 0; codeidx < codes.Length; codeidx++)
            {
                ...
                for (int bar = 0; bar < 8; bar += 2)
                {
                    ...
                    if (barwidth > 0)
                    {
                        result += this.DrawRectangle(true, cursor, 0, barwidth, height);
                    }
        ...
        private string DrawRectangle(bool isDark, int x, int y, int width, decimal height)
        {
            return $"<rect fill='{(isDark ? "black" : "white")}' x='{x}' y='{y}' width='{width}' height='{height}' />" + Environment.NewLine;
        }
    }
}